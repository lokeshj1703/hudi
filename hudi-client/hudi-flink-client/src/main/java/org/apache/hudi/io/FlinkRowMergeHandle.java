 ...
 *
 * @see FlinkRowMergeAndReplaceHandle
 */
public class FlinkRowMergeHandle<T, I, K, O>
    extends HoodieRowMergeHandle<T, I, K, O>
    implements MiniBatchHandle {

  private static final Logger LOG = LoggerFactory.getLogger(FlinkRowMergeHandle.class);
  ...
  public FlinkRowMergeHandle(HoodieWriteConfig config, String instantTime, HoodieTable<T, I, K, O> hoodieTable,
                             MergeContext<T> mergeContext, String partitionPath, String fileId,
                             TaskContextSupplier taskContextSupplier) {
    super(config, instantTime, hoodieTable, mergeContext, partitionPath, fileId, taskContextSupplier, Option.empty());
  ...
  protected void makeOldAndNewFilePaths(String partitionPath, String oldFileName, String newFileName) {
    ...
    try {
      ...
      while (fs.exists(targetFilePath)) {
        ...
        if (fs.getFileStatus(targetFilePath).getLen() <= 0) {
          fs.delete(targetFilePath, false);
          LOG.warn("Delete empty write file for MERGE bucket: " + targetFilePath);
          break;
        }

        rolloverPaths.add(targetFilePath);
        newFileName = newFileNameWithRollover(rollNumber++);
        targetFilePath = makeNewFilePath(partitionPath, newFileName);
        LOG.warn("Duplicate write for MERGE bucket with path: " + oldFilePath + ", rolls over to new path: " + targetFilePath);
      }
    } catch (IOException e) {
      throw new HoodieException("Checking existing path for merge handle error: " + targetFilePath, e);
    }
  }
  ...
  public void finalizeWrite() {
    ...
    try {
      fs.rename(targetFilePath, desiredPath);
    } catch (IOException e) {
      throw new HoodieIOException("Error when rename the temporary roll file: " + targetFilePath + " to: " + desiredPath, e);
    }
  }
  ...
  public void closeGracefully() {
    ...
    try {
      ...
      try {
        fs.delete(targetFilePath, false);
        LOG.info("Deleting the intermediate MERGE data file: " + targetFilePath + " success!");
      } catch (IOException e) {
        // logging a warning and ignore the exception.
        LOG.warn("Deleting the intermediate MERGE data file: " + targetFilePath + " failed", e);
      }
    }
  }
  ...
  public Path getWritePath() {
    return rolloverPaths.size() > 0 ? rolloverPaths.get(0) : targetFilePath;
  }
}
