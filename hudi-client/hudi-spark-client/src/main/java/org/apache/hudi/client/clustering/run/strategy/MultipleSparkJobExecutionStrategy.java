  ...
  private HoodieData<HoodieRecord<T>> readRecordsForGroupWithLogs(JavaSparkContext jsc,
    ...
    return HoodieJavaRDD.of(jsc.parallelize(clusteringOps, clusteringOps.size()).mapPartitions(clusteringOpsPartition -> {
      ...
      clusteringOpsPartition.forEachRemaining(clusteringOp -> {
        ...
        Supplier<ClosableIterator<HoodieRecord<T>>> fileSliceReader = () -> {
          ...
          try {
            ...
            return new HoodieFileSliceReader(baseFileReader, scanner, readerSchema, tableConfig.getPreCombineField(), config.getRecordMerger(),
                tableConfig.getProps(), tableConfig.populateMetaFields() ? Option.empty() : Option.of(Pair.of(tableConfig.getRecordKeyFieldProp(), tableConfig.getPartitionFieldProp())));
          } catch (IOException e) {
            throw new HoodieClusteringException("Error reading input data for " + clusteringOp.getDataFilePath()
                + " and " + clusteringOp.getDeltaFilePaths(), e);
          }
        };
        suppliers.add(fileSliceReader);
      });
  ...
  private HoodieData<HoodieRecord<T>> readRecordsForGroupBaseFiles(JavaSparkContext jsc,
    ...
    return HoodieJavaRDD.of(jsc.parallelize(clusteringOps, clusteringOps.size())
        .mapPartitions(clusteringOpsPartition -> {
          List<Supplier<ClosableIterator<HoodieRecord<T>>>> suppliers = new ArrayList<>();
          clusteringOpsPartition.forEachRemaining(clusteringOp -> {
            Supplier<ClosableIterator<HoodieRecord<T>>> mappingIterator = () -> {
              try {
                ...
                return new CloseableMappingIterator<>(
                    ...
                    rec -> rec.copy().wrapIntoHoodieRecordPayloadWithKeyGen(readerSchema, writeConfig.getProps(), keyGeneratorOp));
              } catch (IOException e) {
                throw new HoodieClusteringException("Error reading input data for " + clusteringOp.getDataFilePath()
                    + " and " + clusteringOp.getDeltaFilePaths(), e);
              }
            };
            suppliers.add(mappingIterator);
          });
          return CloseableIteratorListener.addListener(new LazyConcatenatingIterator<>(suppliers));
        }));
  }
