...
/**
 * Exercises ZookeeperBasedImplicitBasePathLockProvider#getLockBasePath as a pure function — no ZK
 * server needed. Mirrors the DynamoDB implicit-LP normalization contract: the lock base path must
 * be identical regardless of trailing-slash, multi-slash, whitespace, or s3a vs s3 input forms.
 */
class TestZookeeperBasedImplicitBasePathLockProvider {

  private static final String BASE_PATH_WITH_SLASH =
      "s3://dp-datalake-gold-use1-prod/Domain=OfferServed/Table=customerofferserved_v2/";
  private static final String BASE_PATH_NO_SLASH =
      "s3://dp-datalake-gold-use1-prod/Domain=OfferServed/Table=customerofferserved_v2";
