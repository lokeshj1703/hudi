...
public class HoodieAppendHandle<T, I, K, O> extends HoodieWriteHandle<T, I, K, O> {
  ...
  // cores) * hoodie.memory.logfile.append.fraction, floored at
  // HoodieMemoryConfig.MIN_MEMORY_FOR_LOG_APPEND_BUFFER_IN_BYTES (16MB). On Flink the dynamic
  // ceiling is absent and this equals maxBlockSize.
  ...
  private HoodieAppendHandle(HoodieWriteConfig config, String instantTime, HoodieTable<T, I, K, O> hoodieTable,
    ...
    if (dynamicCeiling.isPresent() && dynamicCeiling.get() < this.maxBlockSize) {
      LOG.info(String.format(
          "HoodieAppendHandle for fileId=%s capping block buffer at %d bytes "
              + "(dynamic per-task ceiling) below configured maxBlockSize=%d bytes",
  ...
  /**
   * Routes {@code hoodieRecord} through the insert/update or delete buffering path and returns
   * the record actually appended to {@link #recordList}, or {@code null} for delete-only /
   * ignored / failure windows. See {@link #writeToBuffer} for why the buffered (post-prepare)
   * record is propagated up to {@link #flushToDiskIfRequired} for sizing.
   */
   ...
   * {@link #recordList} — the post-{@link #prepareRecord}/{@link #bufferInsertAndUpdate} clone
   * of {@code populatedRecord} carrying a fully-materialized Avro {@code IndexedRecord} with
   * prepended meta-fields — or {@code null} when nothing was appended to {@code recordList}
   * (delete, ignored, or partition-mismatch failure). The returned reference is what
   * {@link #flushToDiskIfRequired} sizes; sizing the incoming record under-counts heap because
   * the incoming payload is typically still in its compact/deflated wire form, whereas the
   * buffered record holds the fully-deserialized Avro graph plus meta-fields.
   */
  ...
  /**
   * Buffers an insert or update into {@link #recordList} and returns the cloned record actually
   * appended, or {@code null} when the record was ignored. The returned reference is propagated
   * up to {@link #flushToDiskIfRequired} so the EWMA samples the fully-materialized buffered
   * record rather than the (possibly compact/deflated) incoming record.
   */
  private HoodieRecord bufferInsertAndUpdate(Schema schema, HoodieRecord<T> hoodieRecord, boolean isUpdateRecord) throws IOException {
  ...
  protected void flushToDiskIfRequired(HoodieRecord bufferedRecord, boolean appendDeleteBlocks) {
    ...
    if (averageRecordSize > 0 && numberOfRecords >= (long) (effectiveBlockSize / averageRecordSize)) {
   ...
   * path the production call sites take when {@code writeToBuffer} returns null).
   */
  @VisibleForTesting
