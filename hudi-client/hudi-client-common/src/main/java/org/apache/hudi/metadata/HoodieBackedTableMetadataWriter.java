...
public abstract class HoodieBackedTableMetadataWriter<I> implements HoodieTableMetadataWriter {
  ...
  protected final List<MetadataPartitionType> initializedPartitionTypes;
  ...
  protected HoodieBackedTableMetadataWriter(Configuration hadoopConf,
    ...
    this.initializedPartitionTypes = new ArrayList<>(4);
  ...
  private void enablePartitions() {
    ...
    if (dataWriteConfig.isRecordIndexEnabled() || dataWriteConfig.isPartitionedRecordIndexEnabled()
   ...
   * Enable metadata table partitions based on config.
   */
  private void enableInitializedPartitions() {
    final HoodieMetadataConfig metadataConfig = dataWriteConfig.getMetadataConfig();
    if (dataWriteConfig.isMetadataTableEnabled() && dataMetaClient.getTableConfig().isMetadataPartitionAvailable(MetadataPartitionType.FILES)) {
      this.initializedPartitionTypes.add(MetadataPartitionType.FILES);
    }
    if (metadataConfig.isBloomFilterIndexEnabled() && dataMetaClient.getTableConfig().isMetadataPartitionAvailable(MetadataPartitionType.BLOOM_FILTERS)) {
      this.initializedPartitionTypes.add(MetadataPartitionType.BLOOM_FILTERS);
    }
    if (metadataConfig.isColumnStatsIndexEnabled() && dataMetaClient.getTableConfig().isMetadataPartitionAvailable(MetadataPartitionType.COLUMN_STATS)) {
      this.initializedPartitionTypes.add(MetadataPartitionType.COLUMN_STATS);
    }
    if ((dataWriteConfig.isRecordIndexEnabled() || dataWriteConfig.isPartitionedRecordIndexEnabled())
        && dataMetaClient.getTableConfig().isMetadataPartitionAvailable(MetadataPartitionType.RECORD_INDEX)) {
      this.initializedPartitionTypes.add(MetadataPartitionType.RECORD_INDEX);
    }
  }
  ...
  protected boolean initializeIfNeeded(HoodieTableMetaClient dataMetaClient,
    ...
    try {
      ...
      boolean allPartitionsInitialized = initializeFromFilesystem(initializationTime, metadataPartitionsToInit, inflightInstantTimestamp);
      ...
      try {
        switch (partitionType) {
          case FILES:
            ...
            initializeFilegroupsAndCommitToMetadataPartition(partitionType, fileGroupCountAndRecordsPair, commitTimeForPartition);
            break;
          case BLOOM_FILTERS:
          ...
          case COLUMN_STATS:
            ...
            break;
          case RECORD_INDEX:
            initializeFilegroupsAndCommitToRecordIndexPartition(commitTimeForPartition);
            break;
          default:
  ...
  void initializeFilegroupsAndCommitToMetadataPartition(MetadataPartitionType partitionType,
                                                        Pair<Integer, HoodieData<HoodieRecord>> fileGroupCountAndRecordsPair,
                                                        String commitTimeForPartition) throws IOException {
    ...
    initializeFileGroups(dataMetaClient, partitionType, commitTimeForPartition, fileGroupCount, Option.empty(), true);

    // Perform the commit using bulkCommit
    HoodieData<HoodieRecord> records = fileGroupCountAndRecordsPair.getValue();
    bulkCommit(commitTimeForPartition, partitionType, records, new DefaultMetadataTableFileGroupIndexParser(fileGroupCount));
  ...
  private void initializeFilegroupsAndCommitToRecordIndexPartition(String commitTimeForPartition) throws IOException {
    final HoodieTableFileSystemView fsView = new HoodieTableFileSystemView(metadata, dataMetaClient,
        dataMetaClient.getActiveTimeline(), false);
    ...
    if (dataWriteConfig.isPartitionedRecordIndexEnabled()) {
      recordIndexRecords = initializeFilegroupsAndCommitToPartitionedRecordIndexPartition(commitTimeForPartition, fsView, hoodieTable, partitions, latestCommit);
    } else {
      Pair<Integer, HoodieData<HoodieRecord>> fgCountAndRecordIndexRecords = initializeRecordIndexPartition(fsView, hoodieTable, partitions,
          dataWriteConfig.getMetadataConfig().getRecordIndexMaxParallelism(), latestCommit);
      recordIndexRecords = fgCountAndRecordIndexRecords.getRight();
      initializeFilegroupsAndCommitToMetadataPartition(MetadataPartitionType.RECORD_INDEX, fgCountAndRecordIndexRecords, commitTimeForPartition);
    }
    recordIndexRecords.unpersist();
  }

  private HoodieData<HoodieRecord> initializeFilegroupsAndCommitToPartitionedRecordIndexPartition(String commitTimeForPartition, HoodieTableFileSystemView fsView,
                                                                                                  HoodieTable hoodieTable, List<String> partitions, String latestCommit) throws IOException {
    Map<String, Pair<Integer, HoodieData<HoodieRecord>>> fileGroupCountAndRecordsPairMap = new HashMap<>(partitions.size());
    int maxParallelismPerHudiPartition = partitions.isEmpty() ? 1 : Math.max(1, dataWriteConfig.getMetadataConfig().getRecordIndexMaxParallelism() / partitions.size());
    for (String partition : partitions) {
      fileGroupCountAndRecordsPairMap.put(partition, initializeRecordIndexPartition(fsView, hoodieTable, Collections.singletonList(partition), maxParallelismPerHudiPartition, latestCommit));
    }
    ...
    // Generate the file groups
    boolean tryErasePartition = true;
    ...
    for (String dataPartition : fileGroupCountAndRecordsPairMap.keySet()) {
      ...
      initializeFileGroups(dataMetaClient, MetadataPartitionType.RECORD_INDEX, commitTimeForPartition, fileGroupCountAndRecordsPair.getKey(), Option.of(dataPartition), tryErasePartition);
      hoodieDataList.add(fileGroupCountAndRecordsPair.getValue());
      tryErasePartition = false;
    }
    HoodieData<HoodieRecord> records = engineContext.union(hoodieDataList);
    // Perform the commit using bulkCommit
    bulkCommit(commitTimeForPartition, MetadataPartitionType.RECORD_INDEX, records,  new BucketizedMetadataTableFileGroupIndexParser(partitionSizes));
    dataMetaClient.getTableConfig().setMetadataPartitionState(dataMetaClient, MetadataPartitionType.RECORD_INDEX, true);
    // initialize the metadata reader again so the MDT partition can be read after initialization
    initMetadataReader();
    long totalInitTime = partitionInitTimer.endTimer();
  ...
  private Pair<Integer, HoodieData<HoodieRecord>> initializeRecordIndexPartition(HoodieTableFileSystemView fsView, HoodieTable hoodieTable, List<String> partitions,
                                                                                 int maxParallelism, String latestCommit) throws IOException {
    ...
    if (dataMetaClient.getTableConfig().getTableType() == HoodieTableType.COPY_ON_WRITE) {
      // For COW, collect base files directly
      for (String partition : partitions) {
      ...
      // For MOR, extract base files from file slices (we only need base files for estimation)
      List<Pair<String, FileSlice>> latestMergedPartitionFileSliceList = new ArrayList<>();
      for (String partition : partitions) {
    ...
    if (minFileGroupCount != maxFileGroupCount) {
      ...
      fileGroupCount = estimateFileGroupCountFromBaseFiles(partitionBaseFilePairs,
          minFileGroupCount, maxFileGroupCount, hoodieTable);
   ...
   * @param hoodieTable hoodie table instance
  ...
  private int estimateFileGroupCountFromBaseFiles(List<Pair<String, HoodieBaseFile>> partitionBaseFilePairs,
    ...
    long estimatedRecordCount = estimateRecordCountFromBaseFiles(partitionBaseFilePairs, hoodieTable);
    ...
    long totalCount = engineContext.parallelize(partitionBaseFilePairs, parallelism)
        .map(partitionAndBaseFile -> {
          ...
          try {
            ...
            return baseFileUtils.getRowCount(serializableConfiguration.get(), baseFile.getHadoopPath());
          } catch (Exception e) {
  ...
  private List<DirectoryInfo> listAllPartitionsFromMDT(String initializationTime) throws IOException {
    ...
    Path basePath = dataMetaClient.getBasePathV2();
    for (Map.Entry<String, FileStatus[]> entry : partitionFileMap.entrySet()) {
      String relativeDirPath = FSUtils.getRelativePartitionPath(basePath, new Path(entry.getKey()));
      dirinfoList.add(new DirectoryInfo(relativeDirPath, entry.getValue(), initializationTime, false));
    }
    return dirinfoList;
  }
   ...
   *
   * @param tryErasePartition  During initialization, we want to delete any existing files in the partition. However, for partitioned rli we call this method
   *                           multiple times. We guard the deletion code with tryErasePartition and only set it to true the first time this method is called.
   *                           Otherwise, each repeated call would erase the files created by the previous call
   */
  private void initializeFileGroups(HoodieTableMetaClient dataMetaClient, MetadataPartitionType metadataPartition, String instantTime,
                                    int fileGroupCount, Option<String> dataPartitionName, boolean tryErasePartition) throws IOException {
    ...
    final List<String> fileGroupFileIds = IntStream.range(0, fileGroupCount)
        .mapToObj(i -> HoodieTableMetadataUtil.getFileIDForFileGroup(metadataPartition, i, dataPartitionName))
        .collect(Collectors.toList());
  ...
  private static void deletePendingIndexingInstant(HoodieTableMetaClient metaClient, String partitionPath) {
    metaClient.reloadActiveTimeline().filterPendingIndexTimeline().getInstantsAsStream().filter(instant -> REQUESTED.equals(instant.getState()))
        .forEach(instant -> {
          try {
            HoodieIndexPlan indexPlan = metaClient.getActiveTimeline().deserializeInstantContent(instant, HoodieIndexPlan.class);
  ...
  private MetadataRecordsGenerationParams getRecordsGenerationParams() {
    return new MetadataRecordsGenerationParams(
        ...
        metadataWriteConfig.getMaxDFSStreamBufferSize());
  }
  ...
  private Set<String> getMetadataPartitionsToUpdate() {
    ...
    //       Instead of depending on initializedPartitionTypes, the table config becomes the source of truth for which partitions to update.
        ...
        if (!partitionsFromFilesIndex.containsAll(partitionsTouchedByInflightCommit)) {
          for (String partitionToWrite : partitionsTouchedByInflightCommit) {
            if (!partitionsFromFilesIndex.contains(partitionToWrite)) {
              ...
              initializeFileGroups(dataMetaClient, MetadataPartitionType.RECORD_INDEX, instantTime, dataWriteConfig.getPartitionedRecordIndexMinFileGroupCount(), Option.of(partitionToWrite), false);
            }
          }
          initMetadataReader();
        }
  ...
  public void update(HoodieRestoreMetadata restoreMetadata, String instantTime) {
    ...
    try {
      restorePlan = dataMetaClient.getActiveTimeline().deserializeInstantContent(requested, HoodieRestorePlan.class);
    } catch (IOException e) {
  ...
  protected abstract void bulkCommit(
      String instantTime, MetadataPartitionType partitionType, HoodieData<HoodieRecord> records, MetadataTableFileGroupIndexParser indexParser);
  ...
  protected HoodieData<HoodieRecord> prepRecords(Map<MetadataPartitionType,
    ...
    try (HoodieTableFileSystemView fsView = HoodieTableMetadataUtil.getFileSystemView(engineContext, metadataMetaClient)) {
      for (Map.Entry<MetadataPartitionType, HoodieData<HoodieRecord>> entry : partitionRecordsMap.entrySet()) {
        ...
        boolean isPartitionedRLI = Objects.equals(partitionName, MetadataPartitionType.RECORD_INDEX.getPartitionPath()) && dataWriteConfig.isPartitionedRecordIndexEnabled();

        List<FileSlice> fileSlices = HoodieTableMetadataUtil.getPartitionLatestFileSlices(metadataMetaClient, fsView, partitionName);
        ...
        allPartitionRecords = allPartitionRecords.union(
            isPartitionedRLI
                ? prepareRecordsPartitionedRLI(records, fileSlices)
                : prepareRecords(records, entry.getKey(), fileSlices)
        );
      }
  ...
  private HoodieData<HoodieRecord> prepareRecords(HoodieData<HoodieRecord> records, MetadataPartitionType partitionType, List<FileSlice> fileSlices) {
    final int fileGroupCount = fileSlices.size();
    ValidationUtils.checkArgument(fileGroupCount > 0, "FileGroup count for MDT partition " + partitionType.getPartitionPath() + " should be >0");
    List<HoodieRecordLocation> recordLocations  = fileSlices.stream().map(r -> new HoodieRecordLocation(r.getBaseInstantTime(), r.getFileId())).collect(Collectors.toList());
    return records.map(r -> {
      HoodieRecordLocation recordLocation = recordLocations.get(HoodieTableMetadataUtil.mapRecordKeyToFileGroupIndex(r.getRecordKey(),
          fileGroupCount));
      r.unseal();
   ...
   */
  private HoodieData<HoodieRecord> prepareRecordsPartitionedRLI(HoodieData<HoodieRecord> records, List<FileSlice> fileSlices) throws IOException {
    ...
    return records.map(r -> {
      ...
      if (r.getData() instanceof EmptyHoodieRecordPayloadWithPartition) {
        ...
        partitionPath = ((HoodieMetadataPayload) r.getData()).getRecordGlobalLocation().getPartitionPath();
      }
      List<HoodieRecordLocation> recordLocationList = fileSlicesPerHudiPartition.get(partitionPath);
      HoodieRecordLocation recordLocation = recordLocationList.get(HoodieTableMetadataUtil.mapRecordKeyToFileGroupIndex(r.getRecordKey(), recordLocationList.size()));
      r.unseal();
      r.setCurrentLocation(recordLocation);
      r.seal();
      return r;
  ...
  static class DirectoryInfo implements Serializable {
    ...
    public DirectoryInfo(String relativePath, FileStatus[] fileStatus, String maxInstantTime) {
      this(relativePath, fileStatus, maxInstantTime, true);
    }

    public DirectoryInfo(String relativePath, FileStatus[] fileStatus, String maxInstantTime, boolean validateHoodiePartitions) {
