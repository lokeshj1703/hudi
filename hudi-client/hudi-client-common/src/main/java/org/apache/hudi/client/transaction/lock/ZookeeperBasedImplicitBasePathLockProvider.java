...
public class ZookeeperBasedImplicitBasePathLockProvider extends BaseZookeeperBasedLockProvider {
  ...
  public ZookeeperBasedImplicitBasePathLockProvider(final LockConfiguration lockConfiguration, final Configuration conf) {
    ...
    normalizedHudiTableBasePath = normalizeBasePathForLocking(
        lockConfiguration.getConfig().getString(HoodieCommonConfig.BASE_PATH_KEY));
  }

  @Override
  protected String getZkBasePath(LockConfiguration lockConfiguration) {
    String hudiTableBasePath = lockConfiguration.getConfig().getString(HoodieCommonConfig.BASE_PATH_KEY);
