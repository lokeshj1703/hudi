...
public class StreamSync implements Serializable, Closeable {
  ...
  private Pair<Option<String>, JavaRDD<WriteStatus>> writeToSinkAndDoMetaSync(HoodieTableMetaClient metaClient, InputBatch inputBatch,
    ...
    try {
      ...
      String errorTableInstantTime = null;
      if (errorTableWriter.isPresent() && isErrorTableWriteUnificationEnabled) {
        errorTableWriteStatusRDDOpt = errorTableWriter.map(w -> w.upsert(errorTableInstantTime, getLatestCommittedInstant()));
      }
      ...
      WriteStatusValidator writeStatusValidator = new HoodieStreamerWriteStatusValidator(cfg.commitOnErrors, instantTime,
          cfg, errorTableWriter, errorTableWriteStatusRDDOpt, errorWriteFailureStrategy, isErrorTableWriteUnificationEnabled, writeClient,
              latestCommittedInstant, totalSuccessfulRecords);
      String commitActionType = CommitUtils.getCommitActionType(cfg.operation, HoodieTableType.valueOf(cfg.tableType));
  ...
  Map<String, String> extractCheckpointMetadata(InputBatch inputBatch, TypedProperties props, int versionCode, HoodieStreamer.Config cfg) {
    ...
    // Only HoodieIncrSource on v8+ writes V2 cursor; everything else emits V1.
   ...
   * {@link WriteStatus} block a second time. The previous approach issued two separate
   * {@code mapToDouble(...).sum()} actions on the same persisted error-table RDD, which doubled Kryo
   * deserialization of multi-hundred-MB partitions during commit validation and added heap pressure that
   * contributed to executor OOM kills on memory-strained apps (ENG-42105).
   *
   * <p>{@code aggregate} is used (instead of {@code mapPartitions(...).reduce(...)}) so that a 0-partition
   * RDD (e.g. {@code sc.emptyRDD()}, which {@link BaseErrorTableWriter#upsert} can return for an empty
   * commit) returns {@code (0L, 0L)} rather than raising {@code UnsupportedOperationException} as
   * {@code reduce} would. The mutable {@code long[2]} accumulator keeps per-record allocations at zero.
   */
  @VisibleForTesting
