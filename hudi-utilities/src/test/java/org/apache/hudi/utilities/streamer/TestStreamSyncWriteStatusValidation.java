...
/**
 * Tests for {@link StreamSync#sumRecordAndErrorCounts(JavaRDD)}, the single-pass fold that replaced two
 * separate {@code mapToDouble().sum()} actions in error-table commit validation (ENG-42105).
 */
class TestStreamSyncWriteStatusValidation extends SparkClientFunctionalTestHarness {
  ...
  void sumsToZeroForZeroPartitionRDD() {
    // Guards the edge case raised in code review: errorTableWriter.upsert(...) can return an RDD with
    // zero partitions (e.g. sc.emptyRDD()), on which JavaRDD.reduce throws UnsupportedOperationException,
    // whereas the previous mapToDouble(...).sum() returned 0 safely.
