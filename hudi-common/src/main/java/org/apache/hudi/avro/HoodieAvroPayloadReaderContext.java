...
public class HoodieAvroPayloadReaderContext extends HoodieReaderContext<HoodieRecordPayload> {
  ...
  public HoodieAvroPayloadReaderContext(
    ...
    this(storageConfiguration, tableConfig, instantRangeOpt, filterOpt, Collections.emptyMap(), props);
  }
  ...
  public HoodieAvroPayloadReaderContext(
      ...
      TypedProperties props) {
    super(storageConfiguration, tableConfig, instantRangeOpt, filterOpt, new AvroPayloadRecordContext(tableConfig, props));
  ...
  public ClosableIterator<HoodieRecordPayload> getFileRecordIterator(StoragePath filePath, long start, long length, Schema dataSchema,
    ...
    if (keyFilterOpt.isEmpty()) {
      return new CloseableMappingIterator(reader.getIndexedRecordIterator(dataSchema, requiredSchema),
          record -> createPayload(tableConfig.getPayloadClass(), (GenericRecord) record, props));
    }
  ...
  public static HoodieRecordPayload createPayload(String payloadClass, GenericRecord record, TypedProperties props)
      throws HoodieIOException {
    try {
      String[] orderingFields = ConfigUtils.getOrderingFields(props);
      Comparable orderingValue = HoodieRecordUtils.getPreCombineVal(record, orderingFields);
      return (HoodieRecordPayload) ReflectionUtils.loadClass(payloadClass,
          new Class<?>[] {GenericRecord.class, Comparable.class}, record, orderingValue);
    } catch (Throwable e) {
