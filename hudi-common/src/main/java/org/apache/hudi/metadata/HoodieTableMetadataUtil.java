...
public class HoodieTableMetadataUtil {
  ...
  public static Set<String> getValidInstantTimestamps(HoodieTableMetaClient dataMetaClient,
    ...
    final String rollbackFilterThreshold = HoodieTimeline.compareTimestamps(latestMdtCompactionTime,
        HoodieTimeline.GREATER_THAN, earliestInstantTime) ? latestMdtCompactionTime : earliestInstantTime;
    datasetTimeline.getRollbackAndRestoreTimeline().filterCompletedInstants().getInstantsAsStream()
        .filter(instant -> HoodieTimeline.compareTimestamps(instant.getTimestamp(), HoodieTimeline.GREATER_THAN, rollbackFilterThreshold))
        .forEach(instant -> {
